...
from app.core import CIDADE_ESTADOS_ARQUIVO, NOME_ESTADO, NOME_CIDADE, NOME_CIDADE_NORMALIZADO, NOME_BAIRRO, CODE_BAIRRO_COMPLETO, RespostaFormato
...
@router.get("/bairros",
            summary= "Lista os bairros de cidade", 
            description= "Retorna uma lista dos bairros da cidades e estado informado, opcionalmente pode retornar junto o codigo e definir o formato da resposta.")
...
def obter_bairros(
    ...
    colunas = [NOME_ESTADO, NOME_CIDADE, NOME_CIDADE_NORMALIZADO, NOME_BAIRRO]
    if codigos:
        colunas.append(CODE_BAIRRO_COMPLETO)

    # abre o arquivo
    df = pd.read_parquet(CIDADE_ESTADOS_ARQUIVO, columns= colunas)

    # cria o df dos bairros
    df = df[
    ...
    # nao vai fazer nada pq os bairros nao se repetem, pq nao mais dados como ruas ou quadras
    df = df.drop_duplicates(subset=[NOME_BAIRRO])
